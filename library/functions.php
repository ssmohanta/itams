...
function doLogin()
{
	...
	if ($userName == '') {
		...
		if($uType == 'customer')
		{
			$sql = "SELECT  cid, cname, cfullname
					FROM tbl_customer
					WHERE cname = '$userName' AND cpass = PASSWORD('$password')";
			$result = dbQuery($sql);
			if (dbNumRows($result) == 1) {
				$row = dbFetchAssoc($result);
				$_SESSION['user_id'] = $row['cid'];
				$_SESSION['user_name'] = $row['cfullname'];
		...
		{
			$sql = "SELECT  eid, ename, efullname
					FROM tbl_engineer
					WHERE ename = '$userName' AND epass = PASSWORD('$password')";
			$result = dbQuery($sql);
			if (dbNumRows($result) == 1) {
				$row = dbFetchAssoc($result);
				$_SESSION['user_id'] = $row['eid'];
				$_SESSION['user_name'] = $row['efullname'];
				$_SESSION['user_type'] = $uType;
			}//if
			header('Location: /'.WEB_ROOT.'index.php');
			exit;		
	...
	if ($userName == '') {
		...
		if($utype == 'customer'){
			...
			if (dbNumRows($result) == 1) {
				...
				$sql   = "INSERT INTO tbl_customer (cname, cpass, address, c_extension, email, c_mobile, date_time)
						  VALUES ('$userName', '$password', '$txtAdd', '$extn', '$txtEmail','$txtMob',NOW())";
				dbQuery($sql);
